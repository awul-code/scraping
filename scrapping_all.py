    ...
    if pagination:
       ...
       # if next button is not found try to find active page.
        if not next_link:
            ...
            if last_page_btn and 'Selanjutnya' in last_page_btn.text or '>' in last_page_btn.text:
...
with open(data_path, 'w', newline='', encoding='utf-8') as f:
    ...
    while current_url:
        ...
        if not headers_written:
            header_row = [th.get_text(strip=True) for th in 
                           BeautifulSoup(requests.get(current_url, cookies=cookies, headers=headers).text, 'html.parser').select('table thead th')] or [f"Kolom {i+1}" for i in range(len(data[0]))]
